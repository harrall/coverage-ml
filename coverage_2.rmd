...
## 3. Modeling
This chapter desribes the modeling work I did with the dat_all data. First, I normalized the continous variables so that they have mean=0 and a standard deviation of 1. Then, I split the data into a training set and test set. Here's the code:  
...
dat_small <- sample_n(dat_all,1000,replace = FALSE)
...
The accuracy achieved with the kNN method is `r acc_knn`. This result is much better than the parametric LDS and GLM methods, and some 10 percentage points better than guessing. Not bad! But can we do even better?  
